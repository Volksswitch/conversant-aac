...
- **A goal can now come from a place, or from anywhere at all.** Goal buttons used to
  ...
  shows them in this order: what you want from the **person** you picked, what you come
  to the **place** you picked to do, and then your **general** goals — the ones you can
  switch on with anyone, anywhere. A goal that is in two of those lists shows once.
  A place's goals are set in **About Me → My Places** under "What I come here to do";
  the general ones in the new **About Me → Goals For Any Conversation**.
- **Goals now expire with whatever they belonged to.** Change who you are talking with
  and that person's goals switch off; say you are somewhere else and that place's goals
  switch off. A general goal stays on until the conversation ends, because it was never
  about who you were with or where you were.
- Fixed: **how to say a place's name was being forgotten every time the app started.**
  If you had written a respelling for a place — so the voice says it properly — it
  worked for the rest of that session and was blank the next time you opened the app.
  A person's name was never affected.
...
- "Tapping to speak" has moved from the Conversation tab to the "Express Panel and
  ...
  Express Panel, so they now sit on the tab where you set that panel up, just below
  the buttons themselves. Nothing about how they work has changed, and your current
  choice is unaffected.
- There are ten more phrases to choose from in each of the two placeholder lists, so
  what the app says while you are choosing repeats itself less often. They are added
  to the end of your lists; anything you had written or deleted is left exactly as it
  was. New: "I'm working on that", "I'm getting my thoughts together", "I'm figuring
  out what to say", "I'm choosing my words", "I'm thinking it over", "I'm putting a
  reply together", "I need a moment to think", "I'm considering that", "I'm coming up
  with something" and "I'm working out my answer" for what it says first; "Still
  working on it", "I'm still on it", "Still choosing my words", "Still putting it
  together", "Still figuring out what to say", "Still thinking about how to say it",
  "I'm not quite done thinking", "I'm getting closer", "I'm nearly there" and "I'm
  taking a little longer with this one" for while you are still choosing. As always,
  you can edit or remove any of them on the Placeholders tab.
...
- Back to one backup. "Export everything" saves your answers, people, places, buttons,
  ...
  turned out almost nothing genuinely needs holding back, so asking you to manage two
  files was asking you to solve a problem that was not there.
- What little does need holding back is now decided when you import, not when you
  export — because only then does the app know what kind of device it is arriving on.
  Bring a backup to a different kind of device and four settings stay as they are here:
  on-screen or physical keyboard, how the app hears the other person, use the whole
  screen, and the screen edge margin. Bring it to the same kind of device and
  everything comes across. Either way the app tells you which, if any, stayed behind.
- Your answers, people, places, buttons, phrases and conversations always come across
  in full, whatever device the backup came from. Nothing you wrote is ever held back.
- A backup is named "conversant-backup-…" again, and every file the app has ever
  written still restores — including ones you have renamed yourself.
...
- Fixed: your Tester name could go missing on its own. Loading a saved settings
  ...
  It now stays as you set it, whatever you load or restore. If you filled that box
  in once and later found it blank or showing somebody else, this was why.
...
- **Dark** and **High contrast dark** suit a dim room, or anyone bothered by bright
  screens. **Yellow on black** is the highest contrast the screen can produce.
- **Color-blind safe** changes the four kinds of reply so they stay apart for
  red-green color blindness, and gives each one a different edge — solid, double,
  dashed, dotted — so they can be told apart without relying on color at all.
- The app stays on Default unless you change it, and remembers your choice.
- Nothing moves when you switch. Every button and card is in the same place at the
  same size in all seven, so one keyguard fits any of them.
- The "Text Size" tab is now "Text & Color", since it holds both.
- The amber used for the "declining" reply is a shade darker, so the small label on
  that card is easier to read.
...
- Fixed: the first word of a statement could be missing when the app spoke it in
  ...
  before the audio had finished waking up. Whole sentences are spoken now.

- The “How to say it” boxes no longer try to complete what you are typing. A
  respelling is meant to be spelled oddly, so guessing the real word there fought
  you on every letter — and accepting a guess put back the very spelling the voice
  was getting wrong. Word completion is unchanged everywhere else.

- When you send a problem report, the line telling you to read it through first has
  gone. It sat behind the pop-up while it applied and was still on screen afterwards,
  asking you to do something you had already done. The report itself is still shown
  in full for you to read before anything leaves your device.
...
- **The app no longer rewrites what it heard the other person say, and asks the AI one
  ...
  suggestions you were choosing from — and it could quietly reword something that had
  been right. Their words are now recorded exactly as heard. Roughly a third fewer
  requests, and one less thing that can fail.

- **Instead, the app now notes words it thinks may have been misheard.** This costs
  nothing extra — it comes back with the suggestions that were already being requested.
  Settings → Troubleshooting reports how often it happens, broken down by who you were
  with, where you were, and what was listening. Over time that shows which rooms and
  which people the microphone struggles with. Nothing is shown during a conversation
  and nothing changes about the suggestions you are offered.

  Treat the figure as a lowest estimate: a mishearing that still reads like a normal
  sentence — "can" where someone said "can't" — looks like nothing, so it is not
  counted. It is most useful for comparing one setting against another.
...
- The button for answering with a number now always reads **123**. It used to read
  ...
  it opens the keyboard on its number page so you can give any answer you like,
  including one it would never have listed, such as seven and a half.
- That button and the buttons for choices the other person has offered no longer have
  a dashed outline. They are the same kind of button, they both last for a single
  exchange, and they now look the same as each other.
...
- New in Settings → Practice: **A tour of the buttons**. It walks you through the row
  ...
  read, and it waits as long as you like. Press the wrong thing and it tells you what
  the right button looks like and where it sits, then goes on waiting. **It needs no
  API key**, so it is something you can do on your first day, before anything else is
  set up.
...
- Editing an Express Panel button no longer hides the panel. Picking a button used to
  ...
  next to. The cursor now stays out of the box until you tap it, and pressing Enter
  when you have finished typing puts the keyboard away and brings the panel back.
- The fixed color shown on a Partner, Feeling or Place button is now drawn smaller, as
  a sample rather than a button. It looked exactly like the color choices on a phrase
  button, so it invited a tap that could do nothing — those three types have one color
  each and it cannot be changed.
...
- The **Controls** tab is organized into sections too — "Hold on", "Ask them to
  ...
  more thing" each open and close on their own. What you have open stays open while you
  edit, including when you add or reorder cards.
- Sections can hold sections. **Button size**, **Button spacing** and **Minimum
  spacing** now sit together inside **Button sizes and gaps**, with the button that
  resets them at the same indent, so it is clear it puts all three back. The keyguard
  tab's **Title Bar Height** and **Generate Screen Openings** are grouped the same way,
...
- **Add before** and **add after** push everything after the new button along one
  ...
  be no way to reach it again. If that is about to happen the app tells you first,
  naming the button that will be deleted, and you can go ahead or cancel.
- If you already have buttons past the end of the panel from an earlier layout, the tab
  now tells you and offers to clear them.
- **Add API key to Settings** on the opening screen now opens the API Key section for
  you, rather than leaving you to find it among the headings.
- Every setting now sits under a heading, including the few that used to stand on
  their own — so there is nothing loose between the sections, and the **?** button has
  something to tell you about every one of them. The two listening settings, resuming
  automatically and the chime, now share a section called **Listening**.
- The "How I talk with them" section in About Me → People no longer carries a line of
  explanation under its heading. Tap the **?** in the Settings title bar and then a
  heading to hear what that section is for.
...
- Once a week, when you open the app, it now sends us a short report so we can
  ...
  keys**. The Troubleshooting tab lists exactly what is in one, shows when reports
  were sent, and has a switch to turn them off.
- A **Tester name** field on the same tab. It is the only thing that tells us whose
  report is whose, so please leave it as we set it.
...
- **In Practice Mode the other person no longer sounds exactly like you.** If you had never
  ...
  voice. Auto now works out which voice you are actually using and picks a different one,
  preferring another voice in the same language rather than simply the next one on the list.
...
- **Practice Mode — rehearse a conversation with the AI.** On the opening screen, tap
  ...
  person — it speaks their part aloud in its own voice — and you reply by choosing response
  cards, exactly like a real conversation, but with no microphone and no time pressure. Tap
  "Start Listening" to hear the other person's next turn, just as you would in a real chat.
...
- **A clearer, one-at-a-time startup.** The opening screens now appear in order — press
  ...
  API-key note explains that a key is what lets the AI suggest responses, with two buttons:
  "Close" to go straight into the app (listening and speaking in your own words work without
