...
- **Confirm before destroying significant work (standing rule, Ken, June 15 2026).** *Every* action that can wipe away significant user work — clearing/restarting answers, deleting people or relationships, bulk resets, overwriting a saved profile — MUST include an explicit validation step ("Are you sure? This cannot be undone.") before it runs. This is a build checklist item, not an afterthought: when adding any clear/reset/delete/overwrite control, wire its confirmation in the same change. (Trivial, easily-redone actions — a single-field edit/skip with Undo, resetting the usage-token counter — don't require it; the bar is "significant work.") *Audit June 15 2026:* About Me **Restart** ([worldview-ui.js](app/js/worldview-ui.js) `onRestart`) and **Remove person** both confirm; the only un-confirmed destructive action is the Settings usage-counter reset, which is below the bar.
