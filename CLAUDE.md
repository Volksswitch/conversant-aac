...
**OPEN QUESTION — dock the keyboard to one side (Ken, June 14 2026).** Today the keyboard is a **full-width bottom strip** (`position:fixed; left:0; right:0; bottom:0`) and the app reserves `padding-bottom` so the field clears it — it consumes **vertical** space. Ken's insight: **the scarce axis rotates with the device.**
- **Landscape (the likely usage mode):** vertical space is the constraint; a bottom strip eats the scarcest axis. **Dock the keyboard to one side** (left/right) so it consumes the plentiful **horizontal** space instead, leaving the conversation/options full height.
- **Portrait (tablet rotated):** the competition flips to **horizontal**; a **bottom** dock is right again.
- ⇒ The natural design is **orientation-aware docking**: side-dock in landscape, bottom-dock in portrait — switchable automatically (`matchMedia('(orientation: landscape)')` / `visualViewport`) and/or a Settings choice (left / right / bottom / auto). Left-vs-right is itself a handedness/reach preference → a config knob.
- **Coupling to the layout question above:** a row-based QWERTY row is *wide and short* — it docks to the bottom naturally but **not** to a narrow side column. A **side dock pairs with a squarer grid/matrix layout** (e.g. the Alphabetical grid), which also happens to be the better fit for non-touch-typists *and* for the future scanning/eye-gaze renderers. So "dock to the side," "non-QWERTY grid layout," and "scanning-ready geometry" are **one coherent direction**, not three separate features.
- **Access-method framing:** placement (bottom vs. side) and layout (rows vs. grid) are both just **renderer** choices over the same key set — consistent with the engine-emits-descriptor / renderer-presents invariant. Reserving the geometry now keeps the scanning renderer cheap later.
- *Status: recorded for discussion. Implementing it well is coupled to picking the grid layout; recommend deciding layout + docking together rather than retrofitting a side dock onto the row layout. Awaiting Ken's go-ahead.*
