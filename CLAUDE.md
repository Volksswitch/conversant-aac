...
## Worldview Questionnaire (June 2026) — Build Steps 1 & 2 DONE; LLM wiring (Step 3) is next

The worldview model (Architecture Overview §6) has a concrete questionnaire design, a build-ready implementation plan, and **Build Steps 1 (data model + registry) and 2 (questionnaire UI) are now implemented and verified** (June 14 2026).
...
**Remaining build order:** ~~1) data model~~ ✓ → ~~2) questionnaire UI~~ ✓ → **3) LLM integration (structured `{options, missing_facts}`, inject `buildBlock()`, record gaps) ← NEXT** → 4) migrate + remove Name/About You → 5) expand Tier-B + rest of Tier-C → 6) RAG-lite.
...
- Worldview questionnaire: design complete and the five open decisions resolved (June 14 2026); **Build Steps 1 (data model + registry) and 2 (questionnaire UI) implemented and verified**. See the **Worldview Questionnaire (June 2026)** section. Next pick-up is Build Step 3 (LLM integration: inject `buildBlock()`, structured `missing_facts` → gaps).
