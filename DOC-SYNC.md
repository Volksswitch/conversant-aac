...
| Conversant AAC User Manual.docx | ✓ current | 2026-07-08 | `5f672f5` | Reviewed July 8 2026 — already current from the v0.5.60 (July 3) manual-review pass. Fixed: "In my own words" is a compose (pencil) icon now, not a text-labeled button, and standardized "In your own words" → "In my own words" (§4.4 + §7.4, verified against `ui.js`); enhanced the glossary Placeholder entry (plays after anything the partner says). **Re-checked for the 0.5.78 release:** current — placeholder-after-any-turn (glossary), per-conversation privacy (§4.5), "four or eight" palette all present; no repair-of-self card section to update; no new edits. |
