...
const RELEASE_NOTES = {
  "0.7.2": [
    ...
    "The paragraph at the top of a Settings tab that says what the tab is for is now bigger and darker. It was set in the same small grey type as the little status notes, which made it hard to read.",
    ...
    "The Express Panel settings tab now works one button at a time. It used to list every button as a row, which was a lot to wade through and which broke outright when the tab was narrow — the colours and the little arrows ended up on top of each other. Now the tab says \"tap a button in the Express Panel to edit or move it\", and when you tap one you get just that button: what it says, how it sounds, its colour, and buttons to add before, add after, move up, move down, delete and done. Tapping an empty slot still adds a button there. Everything stacks and wraps, so it stays usable however narrow the tab gets.",
    "Deleting a button now asks first, because it also pulls every button after it up one place. Delete is greyed out on an empty button, which has nothing to delete — to be rid of an empty one, use Move down until it reaches the end of the panel.",
