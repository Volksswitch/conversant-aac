...
const RELEASE_NOTES = {
  "0.8.0": [
    ...
    "The Express Panel stays visible while you set it up. Putting the cursor in a phrase box brings the keyboard up over the panel, as it must; tapping anywhere else on the tab now puts the panel straight back. Before this, the keyboard stayed up until you found the small Hide icon, which made the panel impossible to work on.",
    ...
    "The four buttons kept for the other person's choices now say so. They used to sit empty, which looked like something you had forgotten to fill in. They read \"Choice button #1\" and so on in muted text until a choice arrives. You can still tap one to put a partner, place or feeling there - the choices only borrow it for one exchange.",
    "On some layouts the Context band takes two rows whether you ask for one or not. There must always be room for four of the other person's choices, and on a layout whose bottom row is only two buttons wide, one row cannot hold them. The band takes the whole row above rather than half of it, so the bands still line up in straight rows. It can look sparse if you have not filled it, and that is the price of the choices always fitting.",
    "Fixed: asking for two rows of Context gave you one. Two of the side layouts end in a row that holds only the \"In my own words\" key. That row was being counted as one of the rows you asked for, even though it has no buttons in it, so a two-row band came out one row short. Rows are now counted the way you see them: rows of buttons.",
    "Fixed: on some layouts the Context band came out empty. Two of the side layouts end in a row that holds only the \"In my own words\" key, so setting the Context band to one row left it with nowhere to put anything and none of your partners, places or feelings appeared. The band now always keeps at least four buttons, taken from the Always band above it rather than from the Flex band below.",
