...
bulletBold("[PROBE] ", "— still open. A small number of questions remain unanswered and are listed in Section 11."),
callout("Three [RESEARCH] claims in the first draft of this document were WRONG, and the probe caught them. ",
    "Continuous recognition was reported broken on iOS; it works. Safari was reported to return a truncated voice list; it returned 68 voices. The 100vh layout trap was predicted to be a hard failure; it did not reproduce in a Safari tab. This is exactly why the probe was built before anything else, and it is the reason [MEASURED] outranks [RESEARCH] everywhere below."),
emptyPara(),
...
simpleTable(
    ...
    [
        ...
        ["OS keyboard suppression", "inputmode=\"none\" reliable", "Unresolved — see Section 11", { text: "Open", bold: true }],
...
bullet("Distinguish benign session-end errors from fatal ones, so routine churn does not clear listeningIntent. The probe saw an \"aborted\" error on normal teardown, which under the current handler would end listening for the session. [MEASURED — stt.js:252-260]"),
...
boldPara("Remedy: ", "the readonly pattern. Load the field readonly so focus raises no keyboard, and remove the attribute only when the app's own keyboard is not in use. The change is localized to applyInputMode and the IN_SCOPE selector list in keyboard.js."),
callout("This one is NOT resolved, and the probe's method for it was unsound. ",
    "The probe detected the keyboard by watching for a visualViewport height change. Both inputmode=\"none\" and readonly reported exactly 0 px of change — which looks like success, but the control field, which SHOULD raise the keyboard, never completed its measurement in any run. Without the control there is nothing to compare against, and on iPadOS the software keyboard commonly overlays the page WITHOUT resizing the visual viewport, in which case the detection is measuring nothing at all. Treat the keyboard-suppression question as open; resolve it by direct observation on the device rather than by instrumentation. See Section 11. [PROBE]",
    "FBF2E6"),
...
numBold("4 — Layout and touch. ", "The 100dvh fix, clamp recalibration against the measured 763 px usable height, safe-area audit, touch-action, portrait handling, and resolving the keyboard-suppression question by direct observation. Best done with a physical keyguard to check against.", "phases"),
...
para("What the probe did not settle. Four of the original six are now closed — the hosting question, the OPFS write mechanism, the tab-versus-standalone choice, and whether Export/Import should come first — and the answers are in the sections above."),
numBold("Can the operating-system keyboard actually be suppressed? ", "Unresolved, and the probe's method was unsound: it detected the keyboard by watching for a visual-viewport height change, but the control field never completed a measurement, and on iPadOS the keyboard commonly overlays without resizing the viewport at all. Resolve by direct observation — open the three test fields and look. This matters because Conversant draws its own keyboard on the keyguard grid, and the OS keyboard appearing on top of it would be disabling.", "openq"),
