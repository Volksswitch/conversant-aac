...
const doc = new Document({
    ...
    sections: [{
        ...
        children: [
            ...
            para("There are exactly two exceptions, and they are exceptions on purpose: what the partner has just put on the table breaks its band's color, because it is immediate and temporary and should be the thing the eye lands on. The offered alternatives and the number button share that treatment and differ from each other by their edge."),
            lead("Two things are NOT available for any of this, and both come from the app as it stands rather than from preference. ",
                "A ring around a button already means SELECTED, and a dashed border already means the partner just put this on the table. Neither can be borrowed to mean something else. Worth knowing because both are the obvious things to reach for."),
...
,
            ...
            numBold("How the three kinds of Context button are told apart. ", "One background per band is settled (section 5.1); what is not settled is what carries the difference between a partner, a place and a feeling inside that shared background. The bar's color alone proved too weak — brown and olive are near neighbors at five pixels. Four candidates are in the prototype and drawn side by side: the color alone, the bar's thickness, which side it sits on, or a small shape before the label. Thickness is the weakest, because it is a comparative cue — a button on its own tells you nothing. The shape is the strongest and is the most in keeping with the app, which already marks the selected state with a check rather than with color alone; its cost is a little width, which matters most on a three-column side dock.", "open"),
