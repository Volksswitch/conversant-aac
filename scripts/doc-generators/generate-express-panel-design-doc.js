        ...
        children: [
            ...
            simpleTable(
                ...
                [
                    ...
                    ["Choice chips", "Today they take the leading cells of the whole panel and push everything along, so the plumbing is what gets displaced", "Confine them to the context band, as its top and only transient layer (§5.3). This is a fix to current behavior as much as a requirement of the new one"],
            ...
            numBold("Confine the partner's offered alternatives first, on its own. ", "This one is a defect in what is shipped today rather than a piece of the design, and it does not depend on any of the rest. When the other person offers a menu, the alternatives claim the leading cells of the whole panel and push everything along, so a three-way menu moves every phrase three places and drops the last few off the end. Nobody has reported it, which most likely means no tester has yet had a closed-set turn with a full panel. Holding it behind the band work would leave a known fault in place for the sake of tidiness; taking the alternatives from the tail of the list instead of the head can be done now, and the band work later moves them again with no extra cost.", "build"),
