...
,
            ...
            numBold("Ranges — DONE, August 22 2026. ", "A scale or a count now produces one non-speaking button that opens the composer with the number keys showing, and Enter speaks the number. Independent of the bands, so it was built when it was asked for; when the Context band exists the button moves into it beside the offered choices. Two corrections still to make: it should read 123 in every case rather than 1-10 when the ends are known, and the dashed border should come off it and off the offered choices alike.", "build"),
            numBold("The starting set, and the one-time replacement. ", "The shipped phrases, the release that replaces every existing panel with them, and the Reset button that restores them afterwards. Settled before the beta opens and not revisited after it.", "build"),
