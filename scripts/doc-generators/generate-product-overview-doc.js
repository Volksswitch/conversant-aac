...
const doc = new Document({
    ...
    sections: [{
        ...
        children: [
            ...
            para("The system can generate its first response options after as little as half a second of silence — the pause length is adjustable from 0.5 to 3 seconds (two seconds by default) — fast enough to beat the discomfort threshold that research identifies as the point where communication partners begin to disengage."),
            ...
            para("One consequence of speaking as the user is worth stating plainly, because it was a deliberate decision rather than a shortcoming: the app will not answer general-knowledge questions on the user’s behalf. If a communication partner asks what year something happened, what a technical term means, how something works, or what a number works out to, the app does not look the answer up in the AI’s own knowledge and offer it in the user’s voice. It offers what a person offers instead — saying they do not know, turning the question back, or giving what they actually have."),
            ...
            para("3. When the partner asks a question, makes an invitation, or makes a request, a brief, natural placeholder phrase is spoken automatically while the user reads and chooses among the options — “Good question.” The placeholder holds the conversational floor during the choosing window, keeping silence within the one-to-four-second threshold that research shows makes communication partners uncomfortable. If the user picks quickly, no placeholder is needed and none is spoken. Every placeholder is displayed on screen as it plays — the system never speaks on the user’s behalf invisibly."),
            ...
            para("People have goals in every conversation, consciously or not. The system organizes them in three layers by scope — which is also, in effect, by time horizon: a general disposition (how the user likes to come across, across all conversations), standing relationship goals (what the user wants from a specific person, stable across many conversations — “stay close to my sister”), and a goal for the conversation happening right now (“I need to ask Dad for help today”). Each layer shapes which response options are surfaced and how they are ranked, with the most specific layer taking precedence. A goal-setting control integrated with the people editor and the conversation screen is planned for an upcoming release."),
            ...
            para("Every other feature described here assumes a real conversation is already under way, with a real person waiting. That is a demanding place to learn anything. Practice Mode removes the other person: the AI plays the communication partner, speaking their side of a scenario aloud while the user responds exactly as they normally would. No microphone is involved and nobody's patience is being spent."),
            ...
            para("During the beta there is one deliberate exception to this. Test devices send a short report back about once a week — counts and timings describing how the app is being used, together with any errors it recorded — so that problems can be found without a tester having to notice and describe them. A report carries the name assigned to that tester, so it is a record with their name on it rather than an anonymous one. It never contains anything that was said: no transcripts, no partner speech, and neither API key. A transcript is sent only when a tester deliberately attaches one to a problem report, having read it first. Testers can see exactly what a report contains, review every report already sent, and switch the reports off, all from within the app. This applies to the beta group only and ends with the beta."),
            ...
            para("Two optional paid services can be added later, and neither is needed to start. A more natural-sounding voice than the ones built into the device is available on any device, billed by the amount spoken — a few cents an hour of conversation. And on an iPad running from the Home Screen, where Apple does not let the app use the iPad’s own speech recognition, hearing the other person uses the same paid service, billed by the minute of speech. Both draw on a single account and a single key, both are off unless the user turns them on, and the system is fully usable without either — so someone can try everything it does before deciding whether a better voice is worth paying for."),
            ...
            simpleTable([
                ...
                ["Best suited to", "Trying the app, and everyday use by anyone who would rather not add a second paid service.", "Committed, everyday use — particularly for someone already paying for the better-sounding voice, since the same account and key cover both."],
            ]),
            ...
            para("Direct select means the user chooses an item by pointing straight at it, rather than waiting for a cursor to arrive at it. What does the pointing matters less than it might seem. A finger on the screen, a stylus, a mouse, a trackball, an eye gaze camera, or a head-tracking device all arrive at the app the same way, and the app does not ask which one it is dealing with."),
            ...
            bullet("The Express Panel: a customizable panel of one-tap quick phrases, plus Partner and Feeling toggles that tailor openers and suggestions to who the user is with and how they feel"),
            ...
            bullet("Multi-party conversations: tracking who said what when more than one person takes part"),
            ...
            bullet("Social-skills practice: guided practice scenarios for rehearsing conversational responses"),
            ...
            bullet("Symbol and pictographic support: serving users who communicate with symbols rather than text, broadening who the system can serve"),
            heading1("Try It"),
            ...
            para("A brief spoken utterance — for example, “Good question.” or “Still thinking it through.” — spoken automatically by the device to hold the conversational floor while the user reads the response options and chooses. Placeholders prevent the multi-second silence that causes communication partners to disengage. The first is spoken a set interval after the partner stops talking, and any later one says the user is still working on an answer rather than repeating the same phrase. The user controls the timing and how many may be spoken in a single turn, including none at all. Every placeholder is displayed on screen as it plays."),
