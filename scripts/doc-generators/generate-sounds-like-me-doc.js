...
boldPara("The decision: change the question rather than choose a side. ", "The instrument does not ask \"which of these sounds like you?\" (descriptive, and Section 3.3 says they cannot answer it accurately) nor \"which would you rather be?\" (aspirational, and it drifts without a floor). It asks whether a given sentence resembles their own speech:"),
codeBlock(`"Which of these sounds most like something you would say?"

  [ They all sound like me ]     [ I wouldn't say any of these ]`),
boldPara("The exact wording matters, and TWO drafts were rejected before this one. ", "The first read \"which of these would you be happy to have said?\" — rejected because \"happy\" is vague and, worse, admits a threshold reading: it can be heard as \"which of these is acceptable.\" On a satisficing reading several candidates qualify, the user picks the first adequate one, and the instrument stops discriminating. The second read \"which would you rather say?\" — comparative, with no bar to clear, and rejected in turn because in a MADE-UP situation it is faintly nonsensical: the user would not say any of them, because it never happened. Preference also leans aspirational at the moment we want the descriptive answer. Resemblance survives a hypothetical where preference does not."),
boldPara("Why this does not reopen the self-report problem, which is the thing to check before rewording it again. ", "The psychometric objection lands on TRAIT-LEVEL self-description — how formal are you, are you funny — which people genuinely cannot answer accurately. Whether one specific sentence sounds like something you would say is a concrete, item-level judgment, and a far better-behaved question. Any future wording must be comparative rather than a bar to clear, item-level rather than a trait question, and answerable about a situation that did not happen."),
...
boldPara("The same tap works on the Settings Express tab, with no conversation gate (Ken, August 7 2026). ", "The panel renders live and interactive alongside its editor, so tapping a cell edits that cell and a blank cell creates an item in that position. This makes the editor what it should always have been — a view of the panel as it will actually appear, edited in place — and extends the in-situ positioning benefit to the surface where it is unambiguously safe. The conversation gate is unnecessary here for the reason it exists there: in Settings every tap is already deliberate."),
boldPara("This is cheaper than it sounds, because the panel is already there. ", "Checked against the shipped code: nothing hides the Express Panel when Settings opens, and the Settings dialog is deliberately inset to clear the dock — its position is computed from the dock's own width or height plus the separation, so the two never overlap. The panel is therefore already rendered and already visible beside the Settings panel on every tab. It is merely inert, because a modal dialog makes everything outside itself non-interactive."),
boldPara("So the work is not to render the panel there; it is only to make it reachable, and that is a solved problem with an exact precedent. ", "The on-screen keyboard hit the same wall and escapes it by reparenting itself into the open dialog so that it shares the dialog's top layer. The closest precedent is the keyboard's Settings preview mode, which does this with no focused text field at all — a dock element made live inside Settings purely so the user can look at it and change settings against it. That is precisely the Express Panel's case."),
boldPara("The genuine conflict is that the panel and the keyboard want the identical footprint, which is the entire point of the shared grid. ", "On the Express tab both are wanted: the panel at rest, the keyboard once a text field has focus. The dock therefore swaps between them, which is tolerable on a Settings surface in a way it would not be during a conversation. In physical-keyboard mode there is no conflict at all and the panel simply stays up, as the About Me tab already does with the keyboard."),
boldPara("The build hazard, and it is the thing most likely to be got wrong: a tap on the Settings tab is a DIFFERENT ACTION on the same widget, not the same action re-enabled. ", "On the conversation surface, tapping a phrase speaks it, takes the floor, ends any open partner turn, commits a turn to the transcript, and — since August 7 2026 — opens the microphone when it starts a conversation. Every one of those is wrong from a Settings panel. \"Make the panel enabled for that tab\" implemented as \"let the existing handler run\" would speak into a live conversation, commit a turn the user never intended, and start recording, all from inside Settings."),
emptyPara(),
table([2260, 3550, 3550],
    ["", "Conversation surface", "Settings, Express tab"],
    [
        ["Tap a filled cell", "Speaks it as the user's turn", "Edits that item"],
        ["Tap a blank cell", "Nothing (gated during a conversation)", "Creates an item in that position"],
        ["Takes the floor", "Yes — ends any open partner turn", "No"],
        ["Commits to the transcript", "Yes", "No"],
        ["Touches the microphone", "Opens it if this starts a conversation", "Never"],
        ["Preview speech", "n/a — speech is the action", "Optional, and commits nothing"],
    ]),
caption("Table 3. The same button, two contexts. The right-hand column is the whole of what the Express tab may do."),
bulletBold("One thing to decide at build time, not now: ", "a preview tap that speaks the phrase is audible to anyone in the room, including a partner mid-conversation, and it should not enter the transcript, because it is not a turn. That does not violate the rule against speaking on the user's behalf invisibly — the user is looking directly at the text they tapped."),
boldPara("One limitation, now softened rather than merely accepted. ", "The affordance disappears once every cell is filled, so in-situ positioning helps while populating the panel and not while rearranging. Ken's assessment is that this is fine: by the time a user has filled every cell they have internalized what the panel is and how it behaves, and the affordance has done its job. Editing a filled cell in place would need a distinct gesture such as a long press, and adding a gesture to the conversation surface is a larger decision than this one; it is deliberately not proposed here."),
boldPara("The place- and person-scoped panel work cuts both ways here, and it is worth seeing both. ", "It softens the limitation, because each new scope a user defines starts with empty cells — so the affordance keeps earning its place long after the default set is full, rather than retiring once. It also sharpens a question the editor must answer: when a cell is tapped, which scope is being edited? A user who edits what they believe is their clinic panel and in fact edits the global one has been misled by the interface, and the reverse error is worse, because it is silent. The scoped-panel build must make the active scope unmistakable at the moment of editing, not merely selectable somewhere on the tab."),
para("Note that choice chips also claim leading cells dynamically, so a cell that is blank at rest may be occupied during a closed-set turn. The conversation gate already prevents that collision, which is a second thing it buys beyond the safety argument above."),
emptyPara(),
...
caption("Table 4. Phases in recommended order. Phases 0 and 1 together are the minimum that makes the claim honest."),
emptyPara(),
...
bulletBold("Model self-imitation through the harvest. ", "Addressed by the preference-versus-exemplar split in Table 5; it is called out again here because it is invisible when it happens and would be easy to declare a success."),
...
const OUT = process.env.OUT_DOCX || docPath("Conversant AAC Sounds Like Me.docx");
