...
const doc = new Document({
    ...
    sections: [{
        ...
        children: [
            ...
            step("To try it with no service at all, tick \u201cOn this device\u201d - it starts switched off, and the first use downloads a model before anything is spoken or heard. Speaking and hearing are SEPARATE downloads and you are asked about each one on its own; section 1 reports the size of both on this machine. See section 9.", "use"),
            ...
            lead("\u26a0 They are the only two entries that start switched OFF, and that is deliberate. ",
                "The first use downloads the model. Nobody should set that going by pressing a button about voices, so the bench asks first, naming the model and its real size. Afterwards the browser keeps it and there is no further download - which is why the SECOND run is the one to judge, never the first."),
