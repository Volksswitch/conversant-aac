    ...
    sections: [{
        ...
        children: [
            ...
            simpleTable(
                ...
                [
                    ...
                    [{ text: "The device's own", bold: true }, "Yes", "Yes",
                     "Free, built in, no account. The one exception is hearing on an iPad running as an installed app, where this free recognizer returns nothing; paid services are unaffected"],
            ...
            lead("The honest summary. ",
                "Free, adequate, unmeasured, and unavailable in exactly one place: an iPad running the app installed from the Home Screen, where it returns no words at all. That limitation belongs to this free recognizer alone - every paid hearing service was tested on an iPad and every one of them worked, in a Safari tab and in the installed app alike."),
            ...
            para("A ranking on the merits - speed, cost, reach, and how wide a range of voices each company offers. Read the last part of section 8 first: how good a voice sounds has not been judged, and for the speaking half that could reasonably override everything here."),

            heading2("What this ranking deliberately ignores"),
            lead("Which service Conversant can use today is NOT a reason to prefer it. ",
                "Deepgram is the only one wired into the app, for both halves. That is a fact about what has been built so far, not a judgment about the six. For a long time it was believed to be the only one a browser could reach carrying the user's own key; measuring that in September 2026 showed the belief was wrong, and OpenAI, ElevenLabs, Cartesia, Google and Azure all accept the same kind of request. The field is open, so this section ranks the six on what they are worth and treats being built in as a separate fact."),
            lead("It is kept separate rather than dropped. ",
                "Each ranking table below carries a column saying whether the app can reach that service today, and the summary at the end has one column for the best choice on the merits and one for what a user can act on this afternoon. Mixing the two would let the build queue quietly decide which service is best."),
            ...
            simpleTable(
                ...
                [
                    ["Windows, Chromebook or Mac", "Free. The recognizer built into the browser does it", "Optional. Worth paying for if the device's own voices disappoint"],
                    ["Android", "Free. The recognizer built into the browser does it", "Optional, same as above"],
                    ["iPad, in the Safari browser", "Free. The recognizer built into the browser does it", "Strongly wanted. The device's own choice is one ordinary voice or a joke voice"],
                    ["iPad, installed from the Home Screen", "Must be paid for. The free recognizer built into the device returns nothing here. Paid services are unaffected", "Strongly wanted, same as above"],
                ], [2300, 3700, 3360]),
            lead("Be exact about what fails on an installed iPad: it is the FREE recognizer, and only that. ",
                "Apple's own speech recognition, which the browser calls on and which costs nothing, returns no words at all when Conversant is installed from the Home Screen. Every paid hearing service was tested on an iPad in September 2026 and every one of them worked, in a Safari tab and in the installed app alike. So an installed iPad app is the one setup where hearing must be bought - it is not a setup where the choice of who to buy it from is narrowed."),
            lead("A note on the iPad's two modes, because they pull against each other. ",
                "Installing it from the Home Screen makes better use of the screen and keeps your data safely, but it is the one place hearing must be paid for. Running it in a Safari tab hears for free, but the browser can clear your data if the app goes unused for a while. There is no arrangement that gives both."),

            heading2("Hearing: first decide whether to buy it at all"),
            lead("Most people should not. ",
                "The recognizer built into the browser is free, needs no account, and listens while the other person is still talking. It works on Windows, Chromebook, Mac, Android and an iPad in Safari. Its accuracy has never been measured against the paid services, but it has carried this app since the beginning. Two situations make paying worthwhile: an installed iPad app, where it returns nothing; and any user finding it inaccurate in the rooms they are actually in."),
            lead("The other free option is hearing that runs on the machine itself, and on Windows it is genuinely good. ",
                "Measured at 0.33 against the length of the recording, level with the fastest paid service, with nothing sent to any company at all. It is not in the app yet, and it is no good on a Chromebook or on either tablet - but on a laptop it is the most encouraging result in this document."),

            heading2("Hearing, if you are buying it"),
            simpleTable(
                ["", "Choice", "Why", "In the app today"],
                [
                    ["1", "Azure Speech or Deepgram", "Genuinely tied, and split by how much you use. Azure is the fastest measured and free for the first five hours every month; Deepgram has the cheaper rate and is built for listening while somebody talks rather than for taking a finished recording. The crossover is below", "Deepgram only"],
                    ["2", "Google Cloud", "Speed level with Azure, an hour free a month, and $0.96 an hour after that - beaten by Azure on every count that was measured", "No"],
                    ["3", "OpenAI", "The cheapest rate of the four at $0.36 an hour, and the fastest single reading taken - but much the most variable, and it takes a finished recording rather than a live stream, which is the opposite of how this app listens", "No"],
                ], [500, 1900, 5460, 1500]),
            lead("The crossover between Azure and Deepgram is about nine hours of speech a month. ",
                "Azure gives five hours free every month and charges $1.00 an hour after that; Deepgram charges $0.46 an hour from the first minute. Below about nine hours a month Azure costs less - and below five it costs nothing at all - while above nine hours Deepgram's cheaper rate has overtaken Azure's free allowance. Nine hours a month is roughly eighteen minutes a day of the other person actually speaking, and the app only sends audio while somebody is talking, so ordinary use sits well under it. For a light user, Azure. For someone in conversation most of the day, Deepgram."),
            lead("Deepgram's $200 sign-up credit complicates that, and it is worth knowing which way. ",
                "It buys about 435 hours of listening, which at nine hours a month is roughly four years - so a new light user pays nothing on either service for a long time. The difference is what happens at the end of it: Azure's five free hours renew every month and never run out, and a one-time credit by definition does. Judge a long-term choice on the rates rather than on the credit."),
            lead("One accuracy note that belongs in this decision. ",
                "Azure wrote \"testing one two three testing\" as \"testing one 2-3 testing\" on every device and every run, turning two spoken words into a numeric range. Real conversation is full of times, prices and phone numbers, so that habit would be read past regularly. Google and OpenAI both handled it cleanly. It is not enough on its own to unseat Azure, but anybody choosing between the two should know about it."),

            heading2("Speaking, in priority order"),
            para("On speed, cost and range of voices only. How the voices sound is not in this ranking and could reasonably reorder it."),
            simpleTable(
                ["", "Choice", "Why", "In the app today"],
                [
                    ["1", "Azure Speech", "Fastest to start on every device tested, half Deepgram's price, and 500,000 characters free every month that renew and never expire - which covers a light user entirely, forever. Much the widest catalog, including the younger voices this product will need", "No"],
                    ["2", "Deepgram", "Starts speaking in about four tenths of a second and sends the rest in behind, which no other service here does. Twice Azure's price, a one-time credit rather than a renewing allowance, and the narrowest catalog of the six - which is its real weakness rather than its speed", "Yes"],
                    ["3", "Google Cloud", "Middling speed, a large catalog, and the biggest recurring free allowance of any of them. A reasonable third if the first two disappoint", "No"],
                    ["4", "ElevenLabs", "Slow to start and by far the most expensive - and the strongest reputation for how it sounds, which this testing did not check. The one most likely to move up after a listening test", "No"],
                    ["5", "Cartesia", "Similar speed to ElevenLabs and cheaper, with a smaller catalog. Nothing here recommends it or rules it out", "No"],
                    ["6", "OpenAI", "Slowest to start by a wide margin and the least consistent, with eleven voices and no per-language range. Cheap, and that is all", "No"],
                ], [500, 1700, 5660, 1500]),
            lead("Azure wins the speaking half on every measured count. ",
                "Fastest to start, cheapest to run, the only one with a permanently free allowance that covers an ordinary user outright, and the widest range of voices - which is the same property section 10 says answers the younger-voice problem. Every other candidate wins on at most one of those. The one thing that could overturn it is a listening test, and nobody has done one."),
            lead("Deepgram's second place is on the merits, not on being installed. ",
                "It is the only service here that sends audio while it is still making it, so it starts talking far sooner than its total time suggests, and since September 2026 the app plays it as it arrives. What holds it back is price and catalog: twice Azure's rate, no renewing allowance, and the fewest voices of the six."),

            heading2("What this means for the app"),
            lead("The one piece of work this document argues for is adding Azure. ",
                "Not replacing Deepgram - adding a second choice. Deepgram works, it is paid for, it is proven on real devices, and it now starts fast. What Azure adds is a genuinely free option for a light user and a range of voices broad enough for the users this product has not reached yet."),
            lead("What it would cost, stated plainly. ",
                "The app's connection to Deepgram was built once and works; a second service means a second one of those, plus a way to choose between them in the settings and to keep two keys rather than one. It is real work rather than a setting."),
            lead("One asymmetry whoever builds it should know about, which is not a reason to rank Azure lower. ",
                "Deepgram's browser-direct connection is proven in the app on real devices. Azure's is documented by Microsoft, including a browser example that types the key straight into the page, but nobody here has built it. So Azure carries the ordinary risk of anything not yet attempted, and that is a note for the build rather than a mark against the service."),

            heading2("Put together, by setup"),
            simpleTable(
                ["Your setup", "The best choice on the merits", "What you can do today"],
                [
                    ["Windows or Mac", "Free hearing, from the browser or from the machine itself; Azure speaking, free for a light user", "Browser hearing, free. Deepgram speaking if the device voices disappoint"],
                    ["Chromebook", "Free hearing from the browser; Azure speaking. On-device is too slow on this hardware", "Browser hearing, free. Deepgram speaking"],
                    ["Android", "Free hearing from the browser; Azure speaking", "Browser hearing, free. Deepgram speaking"],
                    ["iPad, Safari browser", "Free hearing from the browser; Azure speaking, which matters most here", "Browser hearing, free. Deepgram speaking"],
                    ["iPad, installed app", "Paid hearing - Azure for a light user, Deepgram for a heavy one; Azure speaking", "Deepgram for both. The only setup with no free option at all"],
                ], [1700, 3830, 3830]),
            lead("One line for the whole thing. ",
                "Nobody needs to buy hearing except an installed iPad app; anybody who wants a better voice should buy speaking; and on the merits the service to buy is Azure, which is why teaching the app to use it is the work worth doing next."),
