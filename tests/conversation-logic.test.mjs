...
/* ── Holding newer suggestions until the user is ready (Ken, August 21 2026) ──
 *
 * The regression these guard is a measured one: 47% of the sets of suggestions
 * offered to a real tester were replaced before she touched anything, and her last
 * conversation ended with five sets, nothing chosen, and a message to us.
 */
test('shouldHoldPalette: the first set of a turn is never held', () => {
    // Nothing on screen to disturb, so the user must not be made to ask for it.
    assert.equal(cl.shouldHoldPalette({
        paletteLive: false, shownMode: 'RESPONDING', nextMode: 'RESPONDING',
    }), false);
});

test('shouldHoldPalette: a better set for cards still being read IS held', () => {
    assert.equal(cl.shouldHoldPalette({
        paletteLive: true, shownMode: 'RESPONDING', nextMode: 'RESPONDING',
    }), true);
});

test('shouldHoldPalette: once the user has acted, nothing is held', () => {
    // paletteLive goes false the moment they do anything, so the next set lands
    // normally rather than queueing behind a palette nobody is reading.
    assert.equal(cl.shouldHoldPalette({
        paletteLive: false, shownMode: 'RESPONDING', nextMode: 'RESPONDING',
    }), false);
});

test('shouldHoldPalette: a CHANGE OF MODE always lands, even mid-read', () => {
    // ⚠ The case where holding would be worse than the defect. Goodbyes must not
    // wait behind cards answering a question the partner has finished with, and a
    // partner asking the user to repeat must not be left hanging.
    for (const next of ['PRE_CLOSING_CLOSING', 'REPAIR_OF_SELF', 'INITIATING']) {
        assert.equal(cl.shouldHoldPalette({
            paletteLive: true, shownMode: 'RESPONDING', nextMode: next,
        }), false, `${next} must land immediately`);
    }
});

test('shouldHoldPalette: holds within a mode that is not RESPONDING too', () => {
    assert.equal(cl.shouldHoldPalette({
        paletteLive: true, shownMode: 'REPAIR_OF_SELF', nextMode: 'REPAIR_OF_SELF',
    }), true);
});
