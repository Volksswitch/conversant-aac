...
test('finalize with a null handle APPENDS a finished partner turn (interruption before any pause)', () => {
    ...
    assert.equal(ex.length, 1);
    // cleanedTranscript equals the raw wording on every turn now — the tidy-up pass
    // that used to differ is gone — and the field is kept because every reader of an
    // older conversation still expects it.
    assert.deepEqual(ex[0], { timestamp: ex[0].timestamp, role: 'partner', rawTranscript: 'wait I', cleanedTranscript: 'wait I', uncertain: [], partner: null, place: null, stt: null });
});
